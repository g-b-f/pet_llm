...
def get_storage(n_jobs: int) -> JournalStorage:
    ...
    return JournalStorage(JournalFileBackend(string_backend, lock_obj=lock_obj))


def suggest_vals(
    trial: optuna.Trial, tuner_config: TunerConfig, params_config=ParamsConfig.model_construct()
) -> ParamsConfig:
    
    params_config.temperature = trial.suggest_float(
        "temperature", *tuner_config.temperature
    )
    params_config.frequency_penalty = trial.suggest_float(
        "frequency_penalty", *tuner_config.frequency_penalty
    )
    params_config.presence_penalty = trial.suggest_float(
        "presence_penalty", *tuner_config.presence_penalty
    )
    params_config.repeat_penalty = trial.suggest_float(
        "repeat_penalty", *tuner_config.repeat_penalty
    )
